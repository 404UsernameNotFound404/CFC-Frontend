...
export const UploadPhoto = async (file: any, token: any) => {
    try {
        //get url
        const res = await axios.post(`${BASEURL}/getPreSignedURL`, JSON.stringify({ }), { headers: {"Authorization": Cookie.get("authToken")}});
...
export const updateProfilePhoto = async (link: string) => {
    const res = await axios.post(`${BASEURL}/addProfilePhoto`, JSON.stringify({ Link: link}), { headers: {"Authorization": Cookie.get("authToken")}});
    return res
}